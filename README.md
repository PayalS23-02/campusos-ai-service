[README (1).md](https://github.com/user-attachments/files/32412960/README.1.md)
...
```bash
git clone https://github.com/PayalS23-02/campusos-ai-service.git
cd campusos-ai-service
```
...
```bash
pip install fastapi uvicorn python-multipart opencv-python pytesseract Pillow requests
```
...
> **Note:** The Spring Boot backend currently has open security (`permitAll`) during development. JWT authentication and service-to-service auth will be added once the backend security layer is implemented — this service will be updated accordingly at that time.
...
- No authentication yet on either service — to be added once Spring Boot security (JWT) is implemented.
