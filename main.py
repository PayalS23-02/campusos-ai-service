...
def compare_fields(ocr_text: str, db_record: dict):
    ...
    # Check if the DB name appears in the OCR text (case-insensitive)
    db_name = db_record.get("name", "")
    result["name_match"] = db_name.upper() in ocr_text.upper()
 
    # Roll number match confirms the number is genuinely present in the text
    db_roll = db_record.get("rollNumber", "")
    result["roll_number_match"] = db_roll in ocr_text
 
    # Overall verdict: both must match to be considered fully verified
...
def cross_verify_document(file: UploadFile = File(...)):
    ...
    # Step: find the roll number in the OCR text
    ...
    # Step: look up the student in Prakhar's backend
    ...
    # Step: compare OCR text against the real database record
